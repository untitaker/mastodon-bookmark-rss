...
const launchLogin = async (baseUrl: string) => {
  ...
  const response = await fetch(`https://${baseUrl}/api/v1/apps`, {method: "POST", mode: "cors", body: params});
...
const getFeedUrl = async () => {
  ...
  const response = await fetch(`https://${baseUrl}/oauth/token`, {method: "POST", mode: "cors", body: params});
  const json = await response.json();
...
const App = ({feedUrlPromise}: AppProps) => {
  ...
  if (feedUrl === null) {
    ...
    return <form className="pure-form pure-form-aligned" onSubmit={submitLoginForm}>
      <fieldset>
    ...
    return <div className="green">
      ...
      <form className="pure-form">
        <fieldset>
          <input type="text" className="pure-input-1" readOnly value={feedUrl} />
        </fieldset>
      </form>
    </div>;
  }
